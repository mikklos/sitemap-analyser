(function() {
    ...
    function runWithXmlText(t) {
        ...
        // NY ENKEL OCH ROBUST FUNKTION FÖR DJUPSYNS-VISUALISERING
        function renderDepthVizRobust() {
            var depthVizEl = $('depthViz');
            var depthListEl = $('depthList');
            if (!depthVizEl || !depthListEl) return;
            ...
            if (maxCount === 0) {
                depthVizEl.innerHTML = '<p style="color:#f97316; text-align:center;">Hittade inga URL:er med djupinformation.</p>';
                return;
            }
            
            // 1. Skapa HTML-tabell (Robust Visualisering)
            var tableHtml = '<table class="vizTable"><thead><tr><th>Djup</th><th>URL-antal</th><th style="width: 50%;">Fördelning</th></tr></thead><tbody>';
            ...
            for (var i = 0; i < depthKeys.length; i++) {
                ...
                // HTML Row
                tableHtml += '<tr><td data-depth="' + depth + '"><button class="depthBtn ' + colorClass + '" type="button" data-depth="' + depth + '">Nivå ' + depth + '</button></td>';
                tableHtml += '<td class="depthCount">' + count + ' (' + displayPct + '%)</td>';
                tableHtml += '<td><div class="depthBar ' + colorClass + '" style="width:' + percentage + '%;"></div></td></tr>';
            ...
            depthVizEl.innerHTML = '<p style="font-size:12px; color:#9ca3af; margin-bottom: 8px;">Klicka på en nivå-knapp för att filtrera URL-listan.</p>' + tableHtml;
            
            // 3. Logga till Konsolen (Garanterad visuell feed-back)
            console.log(consoleOutput);

            // 4. Lägg till Eventlyssnare för filtrering (Måste göras efter att elementen skapats)
